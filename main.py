...
@bot_fox.command()
async def test(ctx):
   embed=discord.Embed(title="<#1084000820124713013> ", description="This role allows those to view all gyms within the <#1078515878729093190> category")
  
  
   embed.set_thumbnail(url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTytFGPmFOPoO5jDh8T0-hfUrkKfMXda89w1IRQgeKnng&s")
...
async def j(ctx):
    ...
    voice = get(bot_fox.voice_clients, guild=ctx.guild)
...
async def hug(ctx, *, user: discord.Member = None):
    ...
    if user == bot_fox.user: 
       ...
       embed = discord.Embed(title=f"Hugs {user}",description=f"Hugged by {ctx.author}", color=0xffffff)
...
async def info(ctx,*, member: discord.Member = None):
  ...
  if member == None:
         ...
         embed.add_field(name=f"Roles [{len([role.mention for role in ctx.author.roles [1:]])}]",value=f"{' '+' '.join ([role.mention for role in ctx.author.roles [1:]])}",inline=False)
...
async def bye(ctx):
    ...
    response = f"Good Bye {ctx.author.mention} !"
    await ctx.say(f"{ctx.message.author} said: "+ response)
...
class music(commands.Cog):
    ...
    @commands.command()
    @commands.cooldown(1, 880, commands.BucketType.user)
...
class music(commands.Cog):
    ...
    async def join(self,ctx):
        ...
        if ctx.voice_client is None:
           ...
           await voice_channel.connect()
           await ctx.reply(f"I connected am now channel to the vc channel titled : {voice_channel.mention} ")
        else:
            await ctx.voice_client.move_to(voice_channel)
...
@bot_fox.command()
async def Q(ctx):
    e = discord.Embed(title=" QUESTION", description="When did the dinosours become extinct?")
...
@bot_fox.command()
async def undef(ctx,member: discord.Member):
  await ctx.send(f"{member.mention} is now undefean")
...
def setp(bot):
    bot_fox.add_cog(CommandErrorHandler(bot))
